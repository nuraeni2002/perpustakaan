...
<table class="table table-hover table-striped">
	...
	<tbody>
		...
		while ($data_histori=mysqli_fetch_array($qry_histori)){
			...
			$buku_dipinjam="<ol>";
			$qry_buku=mysqli_query($conn, "select * from detail_peminjaman_buku join buku on buku.id_buku=
			detail_peminjaman_buku.id_buku where id_peminjaman_buku = '".$data_histori['id_peminjaman_buku']."'");
			while ($data_buku=mysqli_fetch_array($qry_buku)){
				$buku_dipinjam.="<li>".$data_buku['nama_buku']."</li>";
			}
			$buku_dipinjam.="</ol>";
			//menampilkan status sudah kembali atau belum
			$qry_cek_kembali=mysqli_query($conn, "select * from pengembalian_buku where id_peminjaman_buku =
			'".$data_histori['id_peminjaman_buku']."'");
			if (mysqli_num_rows($qry_cek_kembali)>0){
				$data_kembali=mysqli_fetch_array($qry_cek_kembali);
				$denda="denda Rp. ".$data_kembali['denda'];
				$status_kembali="<label class='alert alert-succes'>Sudah Kembali <br>".$denda."</label>";
				$button_kembali="";	
			} 
			else {
				$status_kembali="<label class='alert alert-danger'>Belum Kembali</label>";
				$button_kembali="<a href='pengembalian.php?id=".$data_histori['id_peminjaman_buku']."' 
				class='btn btn-warning' onclick='return confirm(\"Yakin ingin mengembalikan?\")'>Kembalikan</a>";
			}
		?>
			<tr>
				<td><?=$no?></td>
